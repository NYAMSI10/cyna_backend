...
export class CouponsService {
  ...
  async create(dto: CreateCouponDto) {
    try {
      ...
      if (exists) {
        return ApiResponse.error('Un coupon avec ce code existe déjà');
      }
  ...
  async update(id: string, dto: UpdateCouponDto) {
    try {
      ...
      const updated = await this.couponModel
        ...
        .exec();
      if (!updated) return ApiResponse.error('Coupon introuvable');
  ...
  async remove(id: string) {
    try {
      ...
      if (!deleted) return ApiResponse.error('Coupon introuvable');
  ...
  async validateCode(code: string, amountHT: number) {
